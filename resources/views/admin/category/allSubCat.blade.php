...
@section('cont1')
    <div class="card card-danger">
        <form method="post" action="{{ route('subCategory.store') }}" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="category_id" id="" value="{{$id}}">
            <div class="card-header">
                <h3 class="card-title">Add new subcategory of {{$category->name}}</h3>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-3">
                    ...
                    <div class="col-4 p-4">
                        <button type="submit" id="add" class="btn btn-outline-success" name="add" value=''>Add
                            subcategory</button>
                    </div>
                </div>
            </div>
...
<tr class="">
                ...
                <td> <div class="form-group col-md-3">
                    <img class="img-thumbnail" src="{{asset('images/subcategories/'.$cat->image)}}" alt="{{$cat->name}}">
                </div></td>
                <td>{{$cat->status}}</td>

                <td>
                    <form method="post" action="{{route('subCategory.destroy',$cat->id)}}">
                        @csrf
                        @method('DELETE')
